 ...
 * full-size blocks.
 */
//DROR here we have the interesting part of finding the currect huff table
METHODDEF(boolean)
decode_mcu (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  if (! entropy->insufficient_data) {
    ...
    /* Outer loop handles each block in the MCU */
    //DROR note blocks in MCU is not 3, it's 6, probably 4 Ys, 1 Cb and 1 Cr
    for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
      JBLOCKROW block = MCU_data[blkn];
      d_derived_tbl * htbl;
      register int s, k, r;
      ...
      /* Decode a single block's worth of coefficients */
      //DROR here is the use of the AC and DC tables
      /* Section F.2.2.1: decode the DC coefficient difference */
      htbl = entropy->dc_cur_tbls[blkn];
      HUFF_DECODE(s, br_state, htbl, return FALSE, label1);
      ...
      if (coef_limit) {
	/* Convert DC difference to actual value, update last_dc_val */
	if (s) {//DROR this means huff faild to decode "simply" and needs the hard work
	  CHECK_BIT_BUFFER(br_state, s, return FALSE);
	  r = GET_BITS(s);
	  s = HUFF_EXTEND(r, s);
	}
	ci = cinfo->MCU_membership[blkn];
	s += state.last_dc_val[ci];//DROR the DC value is saved as the diff from prev val (inited to 0)
	state.last_dc_val[ci] = s;
	/* Output the DC coefficient */
	(*block)[0] = (JCOEF) s;
